...
class DemoMeetingApp {
    constructor() {
        ...
        this.tileIndexToPinEventListener = {};
        ...
        this.videoUpstreamMetricsKeyStats = {
        ...
        this.videoDownstreamMetricsKeyStats = {
        ...
        this.showVideoStats = (tileIndex, keyStatstoShow, metricsData, streamDirection) => {
            ...
            for (const ssrc of streams) {
                for (const [metricName, value] of Object.entries(metricsData[ssrc])) {
                    if (keyStatstoShow[metricName]) {
                        const rowElement = document.getElementById(`${metricName}-${tileIndex}`);
    ...
    initEventListeners() {
        ...
        const speechMonoCheckbox = false; //document.getElementById('fullband-speech-mono-quality');
    ...
    initializeMeetingSession(configuration) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
                ...
                this.meetingSessionPOSTLogger = new amazon_chime_sdk_js_1.MeetingSessionPOSTLogger('SDK', configuration, DemoMeetingApp.LOGGER_BATCH_SIZE, DemoMeetingApp.LOGGER_INTERVAL_MS, `${DemoMeetingApp.BASE_URL}logs`, logLevel);
                this.meetingLogger = new amazon_chime_sdk_js_1.MultiLogger(consoleLogger, this.meetingSessionPOSTLogger);
                this.meetingEventPOSTLogger = new amazon_chime_sdk_js_1.MeetingSessionPOSTLogger('SDKEvent', configuration, DemoMeetingApp.LOGGER_BATCH_SIZE, DemoMeetingApp.LOGGER_INTERVAL_MS, `${DemoMeetingApp.BASE_URL}log_meeting_event`, logLevel);
            }
            ...
            if (this.usePriorityBasedDownlinkPolicy) {
                this.priorityBasedDownlinkPolicy = new amazon_chime_sdk_js_1.VideoPriorityBasedPolicy(this.meetingLogger);
    ...
    setupSubscribeToAttendeeIdPresenceHandler() {
        const handler = (attendeeId, present, externalUserId, dropped) => {
            ...
            if (!this.roster[attendeeId] || !this.roster[attendeeId].name) {
                this.roster[attendeeId] = Object.assign(Object.assign({}, this.roster[attendeeId]), { name: externalUserId.split('#').slice(-1)[0] + (isContentAttendee ? ' «Content»' : '') });
            }
    ...
    createPinUnpinListener(tileState) {
        return (event) => {
            const attendeeId = tileState.boundAttendeeId;
            if (this.roster[attendeeId].pinned) {
                event.target.innerText = 'Pin';
                this.roster[attendeeId].pinned = false;
            }
            else {
                event.target.innerText = 'Unpin';
    ...
    updateDownlinkPreference() {
        ...
        for (const attendeeId in this.roster) {
            if (this.roster[attendeeId].hasVideo) {
                if (this.roster[attendeeId].pinned) {
                    videoPreferences.add(new amazon_chime_sdk_js_1.VideoPreference(attendeeId, 1, amazon_chime_sdk_js_1.TargetDisplaySize.High));
                    this.log(`Pinned video: bwe: new preferences: ${JSON.stringify(videoPreferences)}`);
                }
                else {
                    videoPreferences.add(new amazon_chime_sdk_js_1.VideoPreference(attendeeId, 2, amazon_chime_sdk_js_1.TargetDisplaySize.Low));
    ...
    videoTileDidUpdate(tileState) {
        ...
        const pauseStateElement = document.getElementById(`pause-state-${tileIndex}`);
        ...
        const pinButtonElement = document.getElementById(`video-pin-${tileIndex}`);
    ...
    tileWillBePausedByDownlinkPolicy(tileId) {
        ...
        const attendeeId = (_a = this.audioVideo.getVideoTile(tileId)) === null || _a === void 0 ? void 0 : _a.state().boundAttendeeId;
